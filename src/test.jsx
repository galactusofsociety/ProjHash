...
const App = () => {
 ...
 const styles = {
  ...
  title: {
    ...
    marginBottom: '34px',
    fontSize: '2.8em',
    textShadow: '0 4px 24px #fde04799, 0 2px 8px #fff3',
    fontWeight: 700,
    letterSpacing: '2px',
  },
  controls: {
    ...
    padding: '28px',
    borderRadius: '18px',
    boxShadow: '0 12px 36px rgba(0,0,0,0.7), 0 0 24px #fde04722',
    marginBottom: '28px',
  ...
  btnInsert: {
    ...
    color: '#222',
    borderColor: '#fde047',
    boxShadow: '0 0 15px #fde04760',
  },
  btnSearch: {
    background: '#fff',
    color: '#ffc800',
    borderColor: '#ffc800',
    boxShadow: '0 0 12px #ffc80022',
  },
  btnDelete: {
    background: '#fff',
    color: '#222',
    borderColor: '#222',
    boxShadow: '0 0 11px #22222222',
  },
  btnClear: {
    background: '#fffde4',
    color: '#bb8500',
    borderColor: '#fde047',
    boxShadow: '0 0 10px #fde04755',
  },
  message: {
    ...
    fontWeight: 550,
    color: '#fde047',
    textShadow: '0 0 11px #fde0475a',
    boxShadow: '0 3px 28px #11111160, 0 0 8px #fde04722',
    border: '2px solid #fde047cc',
    opacity: 0,
  ...
  hashInfo: {
    ...
    borderRadius: '12px',
    marginBottom: '28px',
    boxShadow: '0 2px 14px #0a0a0a98',
    border: '1.5px solid #fde047d0',
  },
  hashInfoText: {
    margin: '9px 0',
    color: '#fff',
    fontSize: '15px',
    textShadow: '0 0 4px #fde04722',
  },
  tableWrapper: {
    background: 'rgba(18, 18, 18, 0.97)',
    padding: '36px',
    borderRadius: '17px',
    boxShadow: '0 10px 50px #07070788',
    overflowX: 'auto',
    border: '2px solid #fde04788',
    marginBottom: '0px'
  },
  ...
  bucketAnimating: {
    borderColor: '#fde047',
    background: 'rgba(254, 224, 71, 0.18)',
    boxShadow: '0 0 20px #fde04788',
  },
  bucketProbing: {
    borderColor: '#fff',
  ...
  bucketIndex: {
    ...
    minWidth: '50px',
    textAlign: 'center',
    background: 'rgba(254, 224, 71, 0.12)',
    borderRadius: '9px',
    padding: '10px',
    marginRight: '22px',
    border: '2px solid #fde04744',
    boxShadow: '0 0 10px #fde04744',
  },
  bucketChain: {
    display: 'flex',
    alignItems: 'center',
  ...
  nodeValue: {
    background: 'rgba(254, 224, 71, 0.16)',
    color: '#fde047',
    padding: '11px 19px',
    borderRadius: '11px',
    fontWeight: 700,
  ...
  slotOccupied: {
    borderColor: '#fde047',
    background: 'rgba(254, 224, 71, 0.24)',
    boxShadow: '0 0 18px #fde04755',
  },
  slotAnimating: {
    borderColor: '#fff',
    background: 'rgba(255,255,255,0.07)',
    boxShadow: '0 0 16px #fffde4',
  },
  slotProbing: {
    borderColor: '#fffde4',
    background: 'rgba(255,245,199,0.11)',
  },
  slotIndex: {
    fontWeight: 700,
    fontSize: '16px',
    ...
    padding: '6px 14px',
    marginBottom: '14px',
    border: '2px solid #fde04728',
  },
  slotValue: {
    fontSize: '24px',
    fontWeight: 700,
    color: '#fff',
    textShadow: '0 0 4px #fde04755',
  },
  slotValueEmpty: {
    color: '#fde04733',
    fontSize: '30px',
  ...
  probeIndicator: {
    ...
    fontSize: '14px',
    fontWeight: 700,
    boxShadow: '0 0 12px #fde04790',
    border: '2px solid #fffde4',
    opacity: 0,
    scale: 0,
  },
  probeSequence: {
    background: 'rgba(254, 224, 71, 0.13)',
    color: '#fde047',
    border: '2px solid #fffde4',
    boxShadow: '0 0 24px #fde04733, 0 0 10px #fffde422',
    padding: '17px 22px',
    borderRadius: '12px',
    marginTop: '22px',
    textAlign: 'center',
  ...
  // Animate probe sequence when it appears
  useEffect(() => {
  ...
  const getProbeIndex = (key, i) => {
    ...
    switch (probingMethod) {
      ...
      case 'quadratic':
        return (h1 + i * i) % tableSize;
      case 'double':
        return (h1 + i * hash2(key)) % tableSize;
      default:
        return h1;
    }
  };
  ...
  // Animate bucket/slot highlight
  const animateBucket = (index, type = 'animating') => {
  ...
  const animateSlot = (index, type = 'animating') => {
  ...
  // Animate node insertion/deletion in chain
  ...
  const insertOpen = async (value) => {
    ...
    try {
      ...
      for (let i = 0; i < tableSize; i++) {
        ...
        sequence.push(index);
        setProbeSequence([...sequence]);
        setAnimatingIndex(index);
        animateSlot(index);
        
        if (sequence.length > 1) {
          animateProbeIndicator(index, i);
        }
    ...
    try {
      ...
      for (let i = 0; i < tableSize; i++) {
        ...
        if (table[index].status === 'empty') {
          setMessage(`${key} not found (empty slot)`);
          break;
        }
        if (table[index].value === key && table[index].status === 'occupied') {
          setMessage(`Found ${key} at index ${index} after ${i + 1} probe(s)`);
          found = true;
  ...
  const deleteClosed = async (value) => {
    ...
    try {
      ...
      if (keyIndex !== -1) {
        ...
        if (node) {
          await animateNodeDeletion(node);
        }
    ...
    try {
      ...
      for (let i = 0; i < tableSize; i++) {
        ...
        if (table[index].value === key && table[index].status === 'occupied') {
          ...
          setTable(newTable);
          deleted = true;
          setMessage(`Deleted ${key} from index ${index} after ${i + 1} probe(s)`);
          break;
        }
      }
  ...
  const handleInsert = () => {
    ...
    if (hashingMethod === 'closed') {
      insertClosed(inputValue);
    } else {
      insertOpen(inputValue);
    }
    setInputValue('');
  };
  ...
  const handleDelete = () => {
    if (isOperating || !inputValue) return;
    if (hashingMethod === 'closed') {
      deleteClosed(inputValue);
    } else {
      deleteOpen(inputValue);
    }
    setInputValue('');
  };
  ...
  const getSlotStyle = (slot, index) => {
    ...
    if (slot.status === 'deleted') baseStyle = { ...baseStyle, ...styles.slotDeleted };
  ...
  const isTableReady = () => {
    ...
    if (hashingMethod === 'closed') {
      ...
      return table[0] && typeof table[0] === 'object' && 'status' in table[0];
    }
  };
    ...
    <div style={styles.container}>
      ...
      <div style={styles.controls}>
        <div style={styles.controlGroup}>
          ...
          <input
            ...
            type="number"
            value={tableSize}
            onChange={(e) => setTableSize(Math.max(5, Math.min(20, parseInt(e.target.value) || 10)))}
            min="5"
            max="20"
        ...
        {hashingMethod === 'open' && (
          <div style={styles.controlGroup}>
            ...
            <select
              style={styles.input}
              value={probingMethod}
              onChange={(e) => setProbingMethod(e.target.value)}
              disabled={isOperating}
            >
        ...
        <div style={styles.buttonGroup}>
          <button
            ...
            style={{
              ...styles.btnBase,
              ...styles.btnInsert,
              ...(isOperating ? styles.btnDisabled : {}),
            }}
            disabled={isOperating}
          >
              ...
              {table.map((slot, index) => (
                <div key={index} style={getSlotStyle(slot, index)} data-slot={index}>
                  ...
                  {slot.status === 'occupied' ? (
                    <div style={styles.slotValue} data-slot-value={index}>
                      {slot.value}
                    </div>
                  ) : slot.status === 'deleted' ? (
                    <div
                      style={{ ...styles.slotValue, ...styles.slotValueDeleted }}
                    ...
                    <div
                      style={{ ...styles.slotValue, ...styles.slotValueEmpty }}
                      data-slot-value={index}
                    >
                      —
                    </div>
      ...
      {probeSequence.length > 0 && (
        <div ref={probeRef} style={styles.probeSequence}>
          <strong style={{ color: '#a855f7', marginRight: '10px' }}>
            Probe Sequence:
          </strong>
          {probeSequence.join(' → ')}
        </div>
